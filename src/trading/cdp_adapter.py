...
# --------------------------------------------------------------------------
# ULTIMATE STEALTH SCRIPT: Spoofer WebDriver, WebGL (Apple GPU), Hardware & Canvas
# Dette skjuler at vi er en bot og får os til at ligne en standard Mac-bruger
# --------------------------------------------------------------------------
_CDP_STEALTH_EXTRA_JS: str = """
(function() {
    // 1. Remove obvious webdriver signatures
    try { Object.defineProperty(navigator, 'webdriver', {get: () => undefined}); } catch(e) {}
    window.cdc_adoQpoasnfa76pfcZLmcfl_Array = undefined;
    window.cdc_adoQpoasnfa76pfcZLmcfl_Promise = undefined;
    window.cdc_adoQpoasnfa76pfcZLmcfl_Symbol = undefined;

    // 2. Hardware spoofing (Force Mac Profile)
    ...
    // 3. Canvas fingerprint noise injection (Forhindrer identifikation via rendering)
    const originalFillText = CanvasRenderingContext2D.prototype.fillText;
    CanvasRenderingContext2D.prototype.fillText = function() {
        this.fillStyle = 'rgba(' + Math.floor(Math.random() * 5) + ',0,0,0.005)';
        return originalFillText.apply(this, arguments);
    };

    // 4. WebGL Spoofing (Mask as Apple GPU to hide VPS traces)
    try {
...
def _extract_tradovate_broker_snapshot_from_text(
    ...
    *,
    ...
    snapshot = {
        ...
        "position_open": bool(position_qty is not None and abs(float(position_qty)) > 0.0),
        "position_side": (
            "long" if position_qty is not None and position_qty > 0
            else "short" if position_qty is not None and position_qty < 0
            else "flat"
        ),
        "order_quantity_value": str(order_quantity_value or "").strip(),
        "last_price_text": str(last_label_value or "").strip(),
    ...
    ask_price = snapshot["ask_price"]
    snapshot["spread"] = (
        float(ask_price - bid_price) if bid_price is not None and ask_price is not None else None
    )
    return snapshot
...
class CDPHumanAdapter(BrokerAdapter):
    ...
    def __init__(
        ...
        if not WEBSOCKETS_AVAILABLE:
            raise ImportError("websockets is required for CDPHumanAdapter. Install it with: pip install websockets")
        if not AIOHTTP_AVAILABLE:
            raise ImportError("aiohttp is required for CDPHumanAdapter. Install it with: pip install aiohttp")
    ...
    async def _send_cdp_command(self, method: str, params: dict | None = None) -> dict:
        ...
        try:
            ...
            self._mark_disconnected()
            raise CDPConnectionError("CDP connection was closed. Reconnect to Chrome and try again.") from exc
        ...
        while True:
            try:
                ...
                self._mark_disconnected()
                raise CDPConnectionError("CDP connection closed while waiting for Chrome. Reconnect and try again.") from exc
            data = json.loads(response)
            if data.get("id") == self._message_id:
                if "error" in data:
                    raise CDPConnectionError(f"CDP error: {data['error']}")
    ...
    async def connect(self) -> None:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"http://localhost:{self.cdp_port}/json") as resp:
                    if resp.status != 200:
                        raise CDPConnectionError(f"Cannot connect to Chrome on port {self.cdp_port}.")
                    targets = await resp.json()
            
            ranked_pages = sorted(
                (t for t in targets if t.get("type") == "page"),
                key=lambda t: _score_cdp_page_target(self.platform, str(self._base_url or ""), t),
                reverse=True,
            )
            ...
            if not page_target:
                async with aiohttp.ClientSession() as session:
                    async with session.put(f"http://localhost:{self.cdp_port}/json/new?{self._base_url or 'about:blank'}") as resp:
                        page_target = await resp.json()
                ws_url = str(page_target.get("webSocketDebuggerUrl") or "").strip()

            if not ws_url:
                raise CDPConnectionError("No WebSocket URL found for page.")
    ...
    async def _inject_scripts(self) -> None:
        # Inject cursor overlay first
        js_code = get_full_injection_js()
        await self._send_cdp_command("Page.addScriptToEvaluateOnNewDocument", {"source": js_code})
        await self._send_cdp_command("Runtime.evaluate", {"expression": js_code})

        # Inject Ultimate Stealth
        await self._send_cdp_command("Page.addScriptToEvaluateOnNewDocument", {"source": _CDP_STEALTH_EXTRA_JS})
        await self._send_cdp_command("Runtime.evaluate", {"expression": _CDP_STEALTH_EXTRA_JS})
    ...
    async def _resolve_tradovate_bundle_target(self, selector: str) -> dict[str, Any] | None:
        ...
        (function() {{
            ...
            const isInteractable = (el) => {{
                ...
                const style = window.getComputedStyle(el);
                const className = normalizeText(el.className || '').toLowerCase();
                return !(el.hasAttribute('disabled') || String(el.getAttribute('aria-disabled') || '').toLowerCase() === 'true' || className.includes('disabled') || style.opacity === '0');
            }};
            const scoreModule = (el) => {{
                if (!isVisible(el)) return -1;
                const rect = el.getBoundingClientRect();
                const style = window.getComputedStyle(el);
                const activeClass = /\\b(active|selected|focused|current|primary)\\b/i.test(el.className || '') ? 40 : 0;
                const aria = (el.getAttribute('aria-selected') === 'true' || el.getAttribute('aria-current') || el.getAttribute('data-active') === 'true') ? 35 : 0;
                const focus = el.matches(':focus-within') ? 25 : 0;
                const z = Number.parseInt(style.zIndex || '0', 10);
                const zScore = Number.isFinite(z) ? Math.min(z, 30) : 0;
                const areaScore = Math.min((rect.width * rect.height) / 5000, 25);
            ...
            const findWithinModule = (root, selector) => {{
                try {{
                    if (selector.startsWith('//')) {{
                        const xpath = document.evaluate(selector, root, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
                        for (let i = 0; i < xpath.snapshotLength; i += 1) {{
                            const node = xpath.snapshotItem(i);
                            if (node instanceof Element && isVisible(node)) return node;
                        }}
            ...
            for (const {{ root, label }} of searchRoots) {{
                for (const strategy of ['primary', 'aria', 'semantic']) {{
                    const selectors = Array.isArray(healthConfig.strategies && healthConfig.strategies[strategy]) ? healthConfig.strategies[strategy] : [];
                    for (let index = 0; index < selectors.length; index += 1) {{
                        const candidateSelector = selectors[index];
                        const element = findWithinModule(root, candidateSelector);
                        if (!element) continue;
                        ...
                        return {{
                            ...
                            selector_used: candidateSelector, selector_index: index, strategy,
                            interactable: isInteractable(element), module_selector: label,
                            module_class: root === document ? 'document' : normalizeText((root.className || '')),
                            module_score: label === 'document' ? -1 : scoreModule(root),
                            document_fallback: label === 'document',
                            error_code: isInteractable(element) ? '' : 'ERR_DOM_DISABLED_' + String(healthConfig.action || action || 'ACTION').toUpperCase(),
                        }};
                    }}
                }}
            }}
        ...
        try:
            result = await asyncio.wait_for(
                self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True}),
                timeout=6.0,
            )
        except Exception:
            return None
    ...
    async def _get_element_bounds_basic(self, selector: str) -> dict | None:
        ...
        """
        result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
        return result.get("result", {}).get("value")
    ...
    async def _get_element_bounds(self, selector: str) -> dict | None:
        ...
        if isinstance(bundle_target, dict):
            if not bool(bundle_target.get("interactable", True)):
                return None
            return {"x": bundle_target.get("x"), "y": bundle_target.get("y"), "width": bundle_target.get("width"), "height": bundle_target.get("height")}
        if action is not None:
            return None
        return await self._get_element_bounds_basic(selector)

    async def _get_cursor_overlay_position(self, *, default_x: float = 100.0, default_y: float = 100.0) -> dict[str, float]:
        js_code = f"""
        (function() {{
            if (window[{json.dumps(CURSOR_POS_KEY)}]) return window[{json.dumps(CURSOR_POS_KEY)}];
    ...
    async def _set_cursor_overlay_position(self, x: float, y: float) -> None:
        js_code = f"""
        (function() {{
            const cursor = window[{json.dumps(CURSOR_EL_KEY)}];
            if (cursor) {{ cursor.style.left = {x} + 'px'; cursor.style.top = {y} + 'px'; }}
            window[{json.dumps(CURSOR_POS_KEY)}] = {{ x: {x}, y: {y} }};
        }})();
        """
        await self._send_cdp_command("Runtime.evaluate", {"expression": js_code})

    async def _dispatch_mouse_move(self, x: float, y: float) -> None:
        await self._send_cdp_command("Input.dispatchMouseEvent", {"type": "mouseMoved", "x": float(x), "y": float(y)})
    ...
    async def _move_cursor_human(self, target_x: float, target_y: float, *, duration_ms: float | None = None, should_abort: Callable[[], bool] | None = None) -> bool:
        current = await self._get_cursor_overlay_position(default_x=100.0, default_y=100.0)
        start = Point(x=current["x"], y=current["y"])
        end = Point(x=target_x, y=target_y)
        path = _build_human_cursor_path(start, end, num_points=30, jitter_px=2.0)
    ...
    async def _get_primary_chart_safe_zone(self) -> dict[str, float] | None:
        js_code = f"""
        (function() {{
            const modules = Array.from(document.querySelectorAll({json.dumps(_TRADOVATE_PARENT_MODULE_SELECTOR)}));
            if (!modules.length) return null;
            const rect = modules[0].getBoundingClientRect();
            return {{ x: rect.x, y: rect.y, width: rect.width, height: rect.height }};
        }})();
    ...
    async def reset_ui_focus(self, *, should_abort: Callable[[], bool] | None = None) -> bool:
        ...
        target_x = float(safe_zone["x"]) + float(safe_zone["width"]) * random.uniform(0.72, 0.88)
        target_y = float(safe_zone["y"]) + float(safe_zone["height"]) * random.uniform(0.40, 0.62)
        completed = await self._move_cursor_human(target_x, target_y, duration_ms=random.uniform(1500.0, 3000.0), should_abort=should_abort)
        return bool(completed)

    async def _dispatch_mouse_wheel(self, x: float, y: float, *, delta_x: float = 0.0, delta_y: float = 0.0) -> None:
        await self._send_cdp_command("Input.dispatchMouseEvent", {"type": "mouseWheel", "x": float(x), "y": float(y), "deltaX": float(delta_x), "deltaY": float(delta_y)})

    async def perform_idle_scroll_noise(self) -> bool:
        if not self._ws:
            raise CDPConnectionError("Not connected to Chrome")
        current = await self._get_cursor_overlay_position()
        x, y = float(current.get("x", 100.0)), float(current.get("y", 100.0))
        delta_y = float(random.choice((-14.0, -11.0, -8.0, 8.0, 11.0, 14.0)))
        delta_x = float(random.uniform(-1.5, 1.5))
        await self._dispatch_mouse_wheel(x, y, delta_x=delta_x, delta_y=delta_y)
        await asyncio.sleep(random.uniform(0.05, 0.18))
        await self._dispatch_mouse_wheel(x, y, delta_x=random.uniform(-1.0, 1.0), delta_y=float(-delta_y * random.uniform(0.85, 1.1)))
        return True
    ...
    async def _human_click_selector(self, selector: str) -> bool:
        ...
        if not bounds:
            raise CDPConnectionError(f"Element not found: {selector}")
        offset = random_click_offset(width=bounds["width"], height=bounds["height"], margin_pct=0.2)
        target_x = bounds["x"] + offset.x
        target_y = bounds["y"] + offset.y
        await self._move_cursor_human(target_x, target_y)
        await human_pause(MIN_CLICK_DELAY, MAX_CLICK_DELAY)
        
        await self._send_cdp_command("Input.dispatchMouseEvent", {"type": "mousePressed", "x": target_x, "y": target_y, "button": "left", "clickCount": 1})
        await asyncio.sleep(_human_click_hold_seconds())
        await self._send_cdp_command("Input.dispatchMouseEvent", {"type": "mouseReleased", "x": target_x, "y": target_y, "button": "left", "clickCount": 1})
    ...
    async def _human_type(self, text: str) -> None:
        ...
        delays = human_typing_delays(text, base_wpm=wpm, variance=0.3)
        for char, delay in zip(text, delays):
            await self._send_cdp_command("Input.dispatchKeyEvent", {"type": "keyDown", "text": char})
            await self._send_cdp_command("Input.dispatchKeyEvent", {"type": "char", "text": char})
            await asyncio.sleep(delay)
            await self._send_cdp_command("Input.dispatchKeyEvent", {"type": "keyUp", "text": char})
    
    async def place_order(self, request: OrderRequest) -> OrderResult:
        if not self._connected or not self._selectors:
            return OrderResult(order_id="", status=OrderStatus.ERROR, instrument=request.instrument, side=request.side, quantity=request.quantity, fill_price=None, error_message="Not connected", raw_response={})
        ...
        try:
            ...
            for selector in qty_selectors:
                try:
                    await self._human_click_selector(selector)
                    await human_pause(0.2, 0.5)
                    await self._send_cdp_command("Runtime.evaluate", {"expression": f"document.querySelector('{selector}').value = ''"})
                    await self._human_type(str(int(request.quantity)))
                    break
                except Exception:
                    continue
            ...
            if request.stop_price is not None:
                ...
                for selector in stop_selectors:
                    try:
                        ...
                        continue
                if not stop_filled:
                    raise RuntimeError("KUNNE IKKE INDTASTE STOP-LOSS! Ordre afbrudt for at forhindre ubeskyttet position.")
            ...
            _LOGGER.info(f"[CDPHumanAdapter] Order placed successfully for {instrument_name}")
            return OrderResult(order_id=request.idempotency_key, status=OrderStatus.FILLED, instrument=request.instrument, side=request.side, quantity=request.quantity, fill_price=request.entry_price, error_message=None, raw_response={"instrument_name": instrument_name})
            
        except Exception as e:
            _LOGGER.error(f"[CDPHumanAdapter] Order failed: {e}")
            return OrderResult(order_id="", status=OrderStatus.ERROR, instrument=request.instrument, side=request.side, quantity=request.quantity, fill_price=None, error_message=str(e), raw_response={})
    ...
    async def get_account_balance(self) -> float:
        ...
        try:
            equity_selectors = self._selectors.account_equity.split(", ")
            for selector in equity_selectors:
                js_code = f"""(function() {{ const el = document.querySelector('{selector}'); return el ? (el.innerText || el.textContent) : null; }})();"""
                result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
                text = result.get("result", {}).get("value")
                if text:
                    numbers = re.findall(r'[-+]?\d[\d,.]*', text.replace(',', '.'))
                    if numbers: return float(numbers[0].replace(',', '').replace(' ', ''))
        except Exception:
            pass
        return 0.0
    ...
    async def get_open_positions(self) -> list[dict[str, Any]]:
        if not self._connected or not self._selectors:
            return []
        try:
            js_code = f"""(function() {{ const t = document.querySelector('{self._selectors.positions_table}'); if (!t) return []; return Array.from(t.querySelectorAll('tr')).map(r => ({{text: r.innerText}})); }})();"""
            result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
            return result.get("result", {}).get("value", [])
        except Exception:
            return []
    ...
    async def get_visible_page_text(self) -> str:
        ...
        try:
            js_code = "(function() { const b = document.body; return b ? String(b.innerText || b.textContent || '') : ''; })();"
            result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
            return str(result.get("result", {}).get("value") or "")
        except Exception:
            return ""
    ...
    async def get_visible_page_context(self) -> dict[str, str]:
        if not self._connected: return {"text": "", "title": "", "url": ""}
        try:
            js_code = "(function() { const b = document.body; return { text: String(b ? (b.innerText || b.textContent || '') : ''), title: String(document.title || ''), url: String(window.location.href || '') }; })();"
            result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
            payload = result.get("result", {}).get("value")
            if isinstance(payload, dict): return {"text": str(payload.get("text", "")), "title": str(payload.get("title", "")), "url": str(payload.get("url", ""))}
        except Exception:
            pass
        return {"text": "", "title": "", "url": ""}

    async def get_broker_state_snapshot(self, instrument: str = "", expected_account_tokens: list[str] | tuple[str, ...] = ()) -> dict[str, Any]:
        quantity_value = await self.get_order_quantity_value()
        visible_text = await self.get_visible_page_text()
        instrument_token = str(self._instrument_map.get(instrument, instrument) or "").strip().upper()
        snapshot = _extract_tradovate_broker_snapshot_from_text(visible_text, instrument_token=instrument_token, order_quantity_value=quantity_value, expected_account_tokens=expected_account_tokens)
        rows = await self.get_open_positions()
        snapshot["positions_rows"] = list(rows[:10]) if isinstance(rows, list) else []
        return snapshot

    async def get_tradovate_read_only_snapshot(self, instrument: str = "", expected_account_tokens: list[str] | tuple[str, ...] = (), *, per_call_timeout: float = 7.0) -> dict[str, Any]:
        try:
            page_context = await asyncio.wait_for(self.get_visible_page_context(), timeout=per_call_timeout)
        except Exception:
            page_context = {"text": "", "title": "", "url": ""}

        try:
            quantity_value = await asyncio.wait_for(self.get_order_quantity_value(), timeout=per_call_timeout)
        except Exception:
            quantity_value = None

        instrument_token = str(self._instrument_map.get(instrument, instrument) or "").strip().upper()
        snapshot = _extract_tradovate_broker_snapshot_from_text(page_context.get("text", ""), instrument_token=instrument_token, order_quantity_value=quantity_value, expected_account_tokens=expected_account_tokens)

        try:
            rows = await asyncio.wait_for(self.get_open_positions(), timeout=per_call_timeout)
        except Exception:
            rows = []
    ...
    async def get_position(self, instrument: str) -> dict[str, Any] | None:
        snapshot = await self.get_broker_state_snapshot(instrument)
        if not bool(snapshot.get("position_open")): return None
        if instrument and not bool(snapshot.get("instrument_visible")): return None
        return snapshot

    async def get_order_quantity_value(self) -> str | None:
        if not self._connected or not self._selectors: return None
        try:
            quantity_selectors = self._selectors.order_quantity.split(", ")
            for selector in quantity_selectors:
                js_code = f"""(function() {{ const els = Array.from(document.querySelectorAll('{selector}')); for (const el of els) {{ const rect = el.getBoundingClientRect(); const style = window.getComputedStyle(el); if (rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none') {{ return String(('value' in el) ? el.value : (el.innerText || el.textContent || '')).trim(); }} }} return null; }})();"""
                result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
                value = result.get("result", {}).get("value")
    ...
    async def _set_visible_input_value(self, selector: str, value: str) -> dict[str, Any]:
        desired = str(value)
        js_code = f"""(function() {{ const els = Array.from(document.querySelectorAll('{selector}')); for (const el of els) {{ const rect = el.getBoundingClientRect(); const style = window.getComputedStyle(el); if (rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none') {{ if (typeof el.focus === 'function') el.focus(); const nextValue = {json.dumps(desired)}; const proto = el.tagName === 'TEXTAREA' ? window.HTMLTextAreaElement?.prototype : window.HTMLInputElement?.prototype; const desc = proto ? Object.getOwnPropertyDescriptor(proto, 'value') : null; if (desc && typeof desc.set === 'function') {{ desc.set.call(el, nextValue); }} else if ('value' in el) {{ el.value = nextValue; }} el.dispatchEvent(new Event('input', {{ bubbles: true }})); el.dispatchEvent(new Event('change', {{ bubbles: true }})); if (typeof el.blur === 'function') el.blur(); const applied = ('value' in el) ? el.value : (el.innerText || el.textContent || ''); return {{ ok: String(applied || '').trim() === nextValue, value: String(applied || '').trim() }}; }} }} return {{ ok: false, value: null }}; }})();"""
        result = await self._send_cdp_command("Runtime.evaluate", {"expression": js_code, "returnByValue": True})
        payload = result.get("result", {}).get("value")
    ...
    async def sync_order_quantity(self, quantity: float | int) -> dict[str, Any]:
        if not self._connected or not self._selectors: return {"ok": False, "message": "Not connected"}
        expected = str(int(max(1, round(float(quantity)))))
        current = await self.get_order_quantity_value()
        if current == expected: return {"ok": True, "expected": expected, "value": current, "changed": False}

        for selector in self._selectors.order_quantity.split(", "):
            try:
                bounds = await self._get_element_bounds(selector)
                ...
                await human_pause(0.15, 0.35)
                set_result = await self._set_visible_input_value(selector, expected)
                if not set_result.get("ok"): continue
                await human_pause(0.1, 0.25)
                value_after = await self.get_order_quantity_value()
                if value_after == expected: return {"ok": True, "expected": expected, "value": value_after, "changed": True}
            except Exception: continue

        return {"ok": False, "expected": expected, "value": await self.get_order_quantity_value(), "changed": False}

    async def page_contains_tokens(self, tokens: list[str] | tuple[str, ...]) -> dict[str, Any]:
        if not self._connected: return {"ok": False, "missing": list(tokens)}
        normalized_tokens = [str(token).strip() for token in tokens if str(token).strip()]
        if not normalized_tokens: return {"ok": True, "missing": []}
        try: text = await self.get_visible_page_text()
        except Exception as exc: return {"ok": False, "missing": normalized_tokens, "message": str(exc)}
        haystack = text.casefold()
        missing = [t for t in normalized_tokens if t.casefold() not in haystack]
    ...
    async def close_all_positions(self) -> None:
        ...
        # 1. Forsøg normal Flatten via human cursor click
        if str(getattr(self, "platform", "") or "").strip().lower() == "tradovate":
            for exit_sel in _TRADOVATE_SELECTOR_BUNDLES.get("exit", ()):
                try:
                    clicked = await self.click_element(exit_sel, jitter_px=2.0)
        ...
        # 2. EMERGENCY FALLBACK: Rå JS injection hvis musen/cursor fejler!
        # Dette sikrer at vi altid kommer ud af en handel.
        _LOGGER.warning("[CDPHumanAdapter] Normal exit fejlede. Bruger Rå JS Emergency Flatten.")
        try:
            js_fallback = """
            (function() {
                const btns = document.querySelectorAll('button.btn.btn-default:not(.dropdown-toggle)');
                for (let btn of btns) {
                    if (btn.innerText.includes('Exit at Mkt') || btn.innerText.includes('Flatten')) {
                        btn.click();
                        return true;
                    }
                }
            ...
            """
            await self._send_cdp_command("Runtime.evaluate", {"expression": js_fallback})
        except Exception as e:
            _LOGGER.error(f"[CDPHumanAdapter] Rå JS Emergency Flatten fejlede: {e}")
    ...
    async def get_element_center(self, selector: str) -> tuple[float, float] | None:
        bounds = await self._get_element_bounds(selector)
        if bounds: return bounds["x"] + bounds["width"]/2, bounds["y"] + bounds["height"]/2
        return None

    async def human_click_at(self, x: float, y: float, width: float = 1.0, height: float = 1.0, margin_pct: float = 0.0) -> None:
        if not self._ws: raise CDPConnectionError("Not connected to Chrome")
        target_x = x + random.uniform(-2.0, 2.0)
        target_y = y + random.uniform(-2.0, 2.0)
        if margin_pct > 0.0 and width > 1.0 and height > 1.0:
            offset = random_click_offset(width=width, height=height, margin_pct=margin_pct)
    ...
    async def click_element(self, selector: str, jitter_px: float = 2.0) -> bool:
        ...
        bundle_target = await self._resolve_tradovate_bundle_target(selector)
        if isinstance(bundle_target, dict):
            if not bool(bundle_target.get("interactable", True)): return False
            bx, by, bw, bh = float(bundle_target["x"]), float(bundle_target["y"]), float(bundle_target["width"]), float(bundle_target["height"])
            stealth_range_x = max(jitter_px, bw * 0.25)
            stealth_range_y = max(jitter_px, bh * 0.25)
            cx = bx + bw / 2.0 + random.uniform(-stealth_range_x, stealth_range_x)
            cy = by + bh / 2.0 + random.uniform(-stealth_range_y, stealth_range_y)
        else:
            if action is not None: return False
            centre = await self.get_element_center(selector)
            if centre is None: return False
            cx, cy = centre
            cx += random.uniform(-jitter_px, jitter_px)
            cy += random.uniform(-jitter_px, jitter_px)
    ...
    async def test_connection(self) -> dict[str, Any]:
        if not self._connected or not self._ws: return {"ok": False, "error": "Not connected"}
        try:
            result = await self._send_cdp_command("Target.getTargetInfo")
            return {"ok": True, "url": result.get("targetInfo", {}).get("url", ""), "title": result.get("targetInfo", {}).get("title", "")}
        except Exception as exc: return {"ok": False, "error": str(exc)}

    async def inject_selector_spy(self) -> None:
        if not self._ws: raise CDPConnectionError("Not connected to Chrome")
        spy_js = "(function() { window.lastClickedSelector = null; document.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); window.lastClickedSelector = e.target.tagName.toLowerCase(); }, {once: true, capture: true}); })();"
        await self._send_cdp_command("Runtime.evaluate", {"expression": spy_js, "returnByValue": False})

    async def auto_discover_trading_buttons(self) -> dict[str, dict[str, Any] | None]:
        if not self._ws: raise CDPConnectionError("Not connected to Chrome")
        scan_js = """(function() { function c(s) { var el = document.querySelector(s); return el ? {selector: s, x: el.getBoundingClientRect().x, y: el.getBoundingClientRect().y} : null; } return {buy: c('div.btn.btn-success'), sell: c('div.btn.btn-danger:not(.panic-button)'), flat: c('button.btn.btn-default:not(.dropdown-toggle)')}; })();"""
        result = await self._send_cdp_command("Runtime.evaluate", {"expression": scan_js, "returnByValue": True})
        return result.get("result", {}).get("value") or {}

    async def get_spied_selector(self) -> str | None:
        if not self._ws: raise CDPConnectionError("Not connected to Chrome")
        result = await self._send_cdp_command("Runtime.evaluate", {"expression": "(function() { var s = window.lastClickedSelector; window.lastClickedSelector = null; return s; })();", "returnByValue": True})
        value = result.get("result", {}).get("value")
        return str(value) if value is not None else None
    ...
    def is_connected(self) -> bool:
        ...
        try:
            if hasattr(ws, "closed") and bool(ws.closed): return False
            if hasattr(ws, "open") and not bool(ws.open): return False
